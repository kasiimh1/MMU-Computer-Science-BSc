...
#setup
addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
mult $t0,$t6
mflo $t0
mflo $t1
					...
					loop5:
						...
						loop6:
							...
							loop7:
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop8  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop9  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop10  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop11  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1	
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop12  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1			
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop13  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1			
									...
									addi $t6, $zero, 2 #div mutiplied number #bigger number here results in a slower time
									addi $t1, $t1, -1 #minus one for everytime the loop is run
									bgez $t1, loop14  #loop if the condition is met
									#needs to multi after otherwise the number just increases everytime the loop is called again
									mult $t0,$t6
									mflo $t0
									mflo $t1		
