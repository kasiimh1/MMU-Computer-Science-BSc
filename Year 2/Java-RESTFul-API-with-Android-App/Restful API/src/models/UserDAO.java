...
public class UserDAO {		
	...
	//get specific user access token 
	public User getToken(String token) {
		...
		String query = ("SELECT devtoken FROM users WHERE devtoken = ? ");	
		try {				
			connection = getDBConnection();				
			//prepared statement setting values with setters
			preparedstatement = connection.prepareStatement(query);				
			...
			while (rs.next())
			{	
				v.setToken(rs.getString("devtoken"));
				//close connection when successful
				closeConnection();	
				return v;
			}						
	...
	//get specific user access token 
	public User findToken(String user) {

		/**
		 * SQL query that uses prepared statements (to prevent SQL injection attacks) when manipulating data or talking to the database
		 */

		String query = ("SELECT devtoken FROM users WHERE username = ? ");	
		try {				
			connection = getDBConnection();				
			//prepared statement setting values with setters
			preparedstatement = connection.prepareStatement(query);				
	...
	public Boolean insertUser(User u) throws SQLException {	
		...
		String query = "Insert INTO users (firstname, surname, username, password, user_type, devtoken)"
				+ " VALUES (?,?,?,?,?,?)";	
		try {
			connection = getDBConnection();			
			...
			preparedstatement.setInt(5, u.getUser_type());
			preparedstatement.setString(6, u.getToken());
			preparedstatement.executeUpdate();			
			closeConnection();	
			b = true;
		}
